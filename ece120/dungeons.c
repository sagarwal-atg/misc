   ...
   /***************/
      ...
      while ( room < 3  || room == 5 )	/* Explore the maze */
      {
         if ( room == 0 )
         {
            ...
            printf("an air vent large enough to crawl through and wardrobe\n");
            printf("which leads to a dark hallway.\n" );
            printf("Which exit do you wish to explore?\n");
            printf("Enter 0 for flagstone, 1 for air vent or 2 for wardrobe ");
            scanf("%d", &choice);
            if ( choice == 0 ) { room = 3; }
            if ( choice == 1 ) { room = 1; }
            if ( choice == 2 ) { room = 5; }
        ...
        {
            printf("The dark hallway led to an old room. \n" );
            printf("There was an old man flying in air in a seating position\n" );
            printf("He asks a question: which kind of FSM machine's output \n");
            printf("depends on the input too?\n" );
            printf("If your answer is mealy press 0 and for moore press 1, And \n");
            printf("if you could give an example of what both the machines do press 2\n" );
            scanf("%d" , &choice );
            if( choice == 1)
            {
              printf("The old man electrocutes and sends you to a random place\n");
              room = 3;
            }
            if(choice == 0)
            {
              printf("He says, 'Cooooool' and he sends to a new room using his magic\n" );
              room = 1;
            }
            if( choice == 2)
            {
